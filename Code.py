...
def message_reply(message):
    ...
    if message.text == "Добавить День Рождения":
        ...
        querry = 'select v_birth_note, d_birthday from birthdays where id_telegram like \'{}\''.format(human_id)
        if curs.execute(querry):
    ...
    if ok == 1:
        ...
        querry = 'insert into birthdays values(\'{}\', \'{}\', \'{}\')'.format(human_id, name, data)
        print(querry)
        if curs.execute(querry):
            conn.commit()
